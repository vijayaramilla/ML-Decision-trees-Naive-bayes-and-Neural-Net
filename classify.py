...
def get_args():
	...
	# This optional argument will allow the user to specify whether to use pure information gain (PIG)
    #  or the information gain ratio (IGR) in the decision tree algorithm
	parser.add_argument ("--gain", type=str, required=False, choices=["pig", "igr"], help="For the decision tree, specify either Pure Information Gain (PIG) or Information gain ratio (IGR)")
...
def check_args(args):
	if args.mode.lower() == "train":
		...
		elif args.algorithm.lower() == "decision_tree" and args.gain.lower() != "pig" and args.gain.lower() != "igr":
		    raise Exception("--gain must be properly specified for the decision tree algorithm")
	else:
		if not os.path.exists(args.model_file):
			raise Exception("model file specified by --model-file does not exist.")

def train(instances, algorithm, gain):
	"""
	...
	# Need to be able to accept additional arguments for the different types of 
	#  decision tree tests 
	predictor = None
	
	if algorithm == "decision_tree":
	    predictor = DecisionTree(gain)
...
def main():
	...
	if args.mode.lower() == "train":
		...
		# Train
		predictor = train(instances, args.algorithm, args.gain)
		try:
